...
const image='data:image/png;base64,'+fs.readFileSync(path.join(__dirname,'assets/racing-art.png')).toString('base64');
...
for(const name of ['vendor/three.min.js','race-core.js','track.js','game.js']){
 let source=read(name);if(name==='game.js')source=source.replace('assets/racing-art.png',image);
