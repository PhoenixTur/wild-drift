...
struct Track {
 std::string id;std::vector<Point> points;std::vector<Segment> segments;std::vector<Span> gaps,fences,banks;std::vector<double> pads,pickups,camber;std::vector<Point> tangents;double total=0,width=10.6,fenceLane=10.75;
 void build(){segments.clear();total=0;for(size_t i=0;i<points.size();i++){const auto a=points[i],b=points[(i+1)%points.size()];double len=length(b.x-a.x,b.z-a.z);if(len<.0001)continue;segments.push_back({a,b,total,len,(b.x-a.x)/len,(b.z-a.z)/len});total+=len;}tangents.clear();for(size_t i=0;i<segments.size();i++){const auto& a=segments[(i+segments.size()-1)%segments.size()];const auto& b=segments[i];double n=length(a.tx+b.tx,a.tz+b.tz);tangents.push_back({(a.tx+b.tx)/n,0,(a.tz+b.tz)/n});}}
 ...
 bool bridgeAt(double d)const{d=wrap(d,total);for(const auto& g:gaps)if(d>=g.start-36&&d<=g.end+42)return true;return false;}
 ...
 Surface sample(double d,double lane=0)const{
  d=wrap(d,total);int i=index(d),j=(i+1)%segments.size();const auto& s=segments[i];double t=(d-s.d)/s.len;
  double reach=std::min(.5,4/s.len),wa=std::max(0.,1-t/reach),wb=std::max(0.,1-(1-t)/reach);double tx=s.tx+(tangents[i].x-s.tx)*wa+(tangents[j].x-s.tx)*wb,tz=s.tz+(tangents[i].z-s.tz)*wa+(tangents[j].z-s.tz)*wb,n=length(tx,tz);tx/=n;tz/=n;
  Surface p;p.x=s.a.x+(s.b.x-s.a.x)*t-tz*lane;p.z=s.a.z+(s.b.z-s.a.z)*t+tx*lane;p.y=s.a.y+(s.b.y-s.a.y)*t;p.slope=(s.b.y-s.a.y)/s.len;p.tx=tx;p.tz=tz;p.d=d;p.lane=lane;p.ground=!gapAt(d);
  // Hermite approaches join the road with zero grade change and retain a takeoff lip.
  for(const auto& g:gaps){double u=0,L=0,a=0,b=0,m0=0,m1=0;
   if(d>=g.start-36&&d<g.start){L=36;u=(d-g.start+36)/L;b=5.8;m1=.29;}
   else if(d>=g.start&&d<g.end){p.y+=5.8;continue;}
   else if(d>=g.end&&d<g.end+42){L=42;u=(d-g.end)/L;a=5.8;m0=-.22;}else continue;
   p.y+=(2*u*u*u-3*u*u+1)*a+(u*u*u-2*u*u+u)*L*m0+(-2*u*u*u+3*u*u)*b+(u*u*u-u*u)*L*m1;
   p.slope+=((6*u*u-6*u)*a+(-6*u*u+6*u)*b)/L+(3*u*u-4*u+1)*m0+(3*u*u-2*u)*m1;
  }
  ...
  if(camber.size()==segments.size()){double rate=(camber[j]-camber[i])/s.len,bank=camber[i]+rate*(d-s.d);p.y+=bank*lane;p.crossSlope+=bank;p.slope+=rate*lane;}
  return p;
 }
...
inline Contact overlap(const Racer& a,const Racer& b){
 ...
 auto vertices=[](const Racer& r){auto v=r.shape.hull;if(v.empty())v={{-r.shape.halfWidth,0,-r.shape.rear},{r.shape.halfWidth,0,-r.shape.rear},{r.shape.halfWidth,0,r.shape.front},{-r.shape.halfWidth,0,r.shape.front}};double sn=std::sin(r.yaw),cs=std::cos(r.yaw);for(auto& p:v){double x=p.x,z=p.z;p.x=r.x+x*cs+z*sn;p.z=r.z-x*sn+z*cs;}return v;};
 const auto av=vertices(a),bv=vertices(b);Contact c;c.depth=inf;
 for(const auto* polygon:{&av,&bv})for(size_t i=0;i<polygon->size();i++){
  const auto& p=(*polygon)[i];const auto& q=(*polygon)[(i+1)%polygon->size()];double nx=-(q.z-p.z),nz=q.x-p.x,n=length(nx,nz);if(n<1e-8)continue;nx/=n;nz/=n;
  double amin=inf,amax=-inf,bmin=inf,bmax=-inf;for(const auto& v:av){double d=v.x*nx+v.z*nz;amin=std::min(amin,d);amax=std::max(amax,d);}for(const auto& v:bv){double d=v.x*nx+v.z*nz;bmin=std::min(bmin,d);bmax=std::max(bmax,d);}
  double positive=bmax-amin,negative=amax-bmin;if(positive<=0||negative<=0)return {};double depth=std::min(positive,negative);if(depth<c.depth){double sign=positive<negative?1:-1;c={nx*sign,nz*sign,depth};}
 }return c;
}
...
struct Race {
 const Track* track=nullptr;int driver=0,rank=6,lapCount=3,nextProjectile=0;double time=0,finishTime=0,lastLap=0,driftSeconds=0;int jumps=0,hits=0;bool finished=false,rewardClaimed=false;uint32_t rng=48329;std::vector<Racer> racers=std::vector<Racer>(6);std::vector<Pickup> pickups;std::vector<Projectile> projectiles;std::vector<Effect> effects;std::vector<double> lapTimes;std::string notice;
 void reset(const Track& t,int hero=0,const std::array<int,5>& upgrades={},int laps=3,int opponents=5){*this=Race{};track=&t;driver=std::clamp(hero,0,heroCount-1);lapCount=std::clamp(laps,1,10);racers.resize(std::clamp(opponents,0,heroCount)+1);rank=int(racers.size());for(int i=0;i<int(racers.size());i++){auto& r=racers[i];r.id=i;r.hero=(driver+i)%heroCount;r.cfg=stats(r.hero,i==0?upgrades:std::array<int,5>{});double d=i==0?0:9+(int(racers.size())-1-i)*6,lane=i==0?-2.5:(i%2?2.5:-2.5);place(r,d,0,lane);}for(double d:t.pickups)pickups.push_back({d,0});}
 void place(Racer& r,double d,double speed=0,double lane=0){auto p=track->sample(d,lane);r.x=p.x;r.y=p.y;r.z=p.z;r.yaw=std::atan2(p.tx,p.tz);r.vx=p.tx*speed;r.vz=p.tz*speed;r.vy=0;r.speed=speed;r.d=d;r.lane=lane;r.nearD=p.d;r.surface=p;r.lastSafeD=d;r.grounded=true;r.phase=Phase::Driving;r.phaseTime=0;r.yawRate=r.steering=r.driftBlend=r.drift=0;r.drifting=false;r.jumpOrigin=nan;}
 ...
 void hit(Racer& r,double duration){if(r.phase!=Phase::Driving)return;if(r.shield>0){if(r.id==0)notice="ЩИТ ОТРАЗИЛ УДАР";return;}r.stun=r.hitDuration=duration*r.cfg.armor;r.vx*=.42;r.vz*=.42;r.speed=length(r.vx,r.vz);r.hitAnim=.7;r.drift=r.boost=0;r.drifting=false;if(r.id==0)notice="ПОПАДАНИЕ · СКОРОСТЬ СНИЖЕНА";}
 ...
 void step(Input input,double dt){if(finished||!track)return;dt=clamp(dt,0,.05);time+=dt;auto& p=racers[0];for(auto& r:racers){for(double* timer:{&r.boost,&r.shield,&r.stun,&r.contact,&r.wallContact,&r.throwAnim,&r.hitAnim,&r.landing})*timer=std::max(0.,*timer-dt);r.itemAge+=dt;drive(r,r.id==0?input:botInput(r),dt);if(r.id>0&&r.item!=Item::None&&r.itemAge>2.8&&r.phase==Phase::Driving)useItem(r.id);}
  ...
  for(int pass=0;pass<4;pass++)for(int i=0;i<int(racers.size());i++)for(int j=i+1;j<int(racers.size());j++){
  ...
  tickProjectiles(dt);int laps=std::min(lapCount,int(std::floor(std::max(0.,p.d)/track->total)));if(laps>int(lapTimes.size())&&p.checkpoint>=laps*12){lapTimes.push_back(time-lastLap);lastLap=time;if(laps<lapCount)notice=laps==lapCount-1?"ПОСЛЕДНИЙ КРУГ!":"НОВЫЙ КРУГ";}rank=1;for(int i=1;i<int(racers.size());i++)if(racers[i].d>p.d||std::isfinite(racers[i].finishTime))rank++;if(std::isfinite(p.finishTime)){finished=true;finishTime=p.finishTime;rank=1;for(int i=1;i<int(racers.size());i++)if(racers[i].finishTime<finishTime)rank++;}
 }
 Reward award(Profile& profile,int course){if(!finished||rewardClaimed)return {};auto& h=profile.heroes[driver];Reward r;int old=h.xp/150;r.xp=int(std::round((140+(int(racers.size())-rank)*28+std::min(50.,driftSeconds)+jumps*8+hits*12)*stats(driver).xpBonus));r.points=100+(int(racers.size())-rank)*25+hits*10;h.xp=std::min(1000000,h.xp+r.xp);profile.points=std::min(100000000,profile.points+r.points);r.levelUps=h.xp/150-old;if(lapCount==3)profile.records[course]=std::min(profile.records[course],finishTime);rewardClaimed=true;return r;}
};
}
