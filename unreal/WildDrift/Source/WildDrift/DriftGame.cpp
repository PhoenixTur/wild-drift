...
void AWildDriftGameMode::BeginPlay(){
 Super::BeginPlay();Catalog=ReadJson(TEXT("catalog.json"));if(!Catalog||Catalog->GetIntegerField(TEXT("schema"))!=1){Fail(TEXT("Missing PortData/catalog.json"));return;}Gallery=FParse::Param(FCommandLine::Get(),TEXT("WildDriftGallery"));Verify=FParse::Param(FCommandLine::Get(),TEXT("WildDriftVerify"));Tracks=drift::authoredTracks();LoadMaterials();if(!Error.IsEmpty())return;LoadProfile();if(Gallery)Profile.heroes[0].skin=0;CameraActor=GetWorld()->SpawnActor<AActor>();Camera=NewObject<UCameraComponent>(CameraActor);CameraActor->SetRootComponent(Camera);Camera->RegisterComponent();Camera->SetFieldOfView(64);Camera->PostProcessSettings.bOverride_MotionBlurAmount=true;Camera->PostProcessSettings.MotionBlurAmount=0;
 ...
 GetWorld()->GetFirstPlayerController()->SetViewTarget(CameraActor);SmokeSuite=FParse::Param(FCommandLine::Get(),TEXT("WildDriftSuite"));SmokeTest=SmokeSuite||FParse::Param(FCommandLine::Get(),TEXT("WildDriftSmoke"));AutoDrive=SmokeTest||FParse::Param(FCommandLine::Get(),TEXT("WildDriftAutoDrive"));FParse::Value(FCommandLine::Get(),TEXT("WildDriftCourse="),Course);Course=FMath::Clamp(Course,0,4);FParse::Value(FCommandLine::Get(),TEXT("WildDriftLaps="),Laps);FParse::Value(FCommandLine::Get(),TEXT("WildDriftOpponents="),Opponents);Laps=FMath::Clamp(Laps,1,10);Opponents=FMath::Clamp(Opponents,0,drift::heroCount);LoadCourse();if(Verify){Profile={};Profile.chooseSkin(Hero,1);Profile.upgrade(Hero,0);SaveProfile();Profile={};LoadProfile();if(Profile.points!=40||Profile.heroes[Hero].skin!=1||Profile.heroes[Hero].upgrades[0]!=1){Fail(TEXT("Verification: profile round-trip failed"));FPlatformMisc::RequestExitWithStatus(false,1);return;}}if(SmokeTest||AutoDrive||Verify)StartRace();else if(!Gallery)ShowMenu();
}
...
void AWildDriftGameMode::CreateKarts(){for(auto K:Karts)if(K)K->Destroy();Karts.Empty();Race.reset(Tracks[Course],Hero,Profile.heroes[Hero].upgrades,Laps,Opponents);Previous=Race.racers;
 for(int I=0;I<int(Race.racers.size());I++){int H=Race.racers[I].hero;Race.racers[I].shape=drift::authoredShape(H,Profile.heroes[H].skin);auto* K=LoadModel(FString::Printf(TEXT("hero-%d-skin-%d.json"),H,Profile.heroes[H].skin));Karts.Add(K);if(K)K->SetActorHiddenInGame(!Racing&&I>0);}
...
void AWildDriftGameMode::StartRace(){Racing=true;Paused=false;CreateKarts();Countdown=AutoDrive||Verify?0:3;Reward={};NoticeAge=0;HideMenu();auto* PC=GetWorld()->GetFirstPlayerController();PC->SetInputMode(FInputModeGameOnly());PC->bShowMouseCursor=false;UE_LOG(LogTemp,Display,TEXT("WildDrift: started course %d with hero %d, laps %d, opponents %d"),Course,Hero,Race.lapCount,int(Race.racers.size())-1);}
...
void AWildDriftGameMode::Tick(float Delta){
 Super::Tick(Delta);if(!Error.IsEmpty()||!Camera||Karts.Num()!=int(Race.racers.size()))return;double Dt=FMath::Min(double(Delta),.1);Clock+=Dt;if(Verify)VerifyInput();auto* PC=GetWorld()->GetFirstPlayerController();if(PC->WasInputKeyJustPressed(EKeys::M))AudioMuted=!AudioMuted;UpdateAudio();if(PC->WasInputKeyJustPressed(EKeys::Escape)){if(Racing&&!Race.finished)TogglePause();else if(!Racing&&MenuPage){MenuPage=0;ShowMenu();}}
 ...
 if(Gallery){
  ...
  if(Clock>3.5){GalleryStage++;if(GalleryStage>=48){UE_LOG(LogTemp,Display,TEXT("WILDDRIFT_GALLERY_OK 24 vehicle variants, front and rear views"));FPlatformMisc::RequestExit(false);}else{Hero=GalleryStage/6;int Skin=(GalleryStage/2)%3;Profile.heroes[Hero].skins[Skin]=true;Profile.heroes[Hero].skin=Skin;CreateKarts();SmokeFinished=false;Clock=0;}}
 }
...
void AWildDriftGameMode::UpdateScene(double Dt){
 double Alpha=Racing?Accumulator*120:1;for(int I=0;I<int(Race.racers.size());I++){if(!Karts[I])continue;const auto& R=Race.racers[I];const auto& Old=Previous[I];FVector Pos=FMath::Lerp(Position(Old),Position(R),Alpha);double Yaw=Old.yaw+drift::angle(R.yaw-Old.yaw)*Alpha;auto& Pose=Karts[I]->Attitude;Pose.update(R,Tracks[Course],Paused?0:Dt);
