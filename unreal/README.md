...
Нативная версия 0.4 для Apple silicon. В этой папке отдельный C++ проект UE 5.8; браузерная игра в корне репозитория остаётся самостоятельной.
...
Пять трасс различаются декорациями и погодой: пепельный ветер, дождливый лес, ледяной серпантин с метелью, кузня с шестернями и искрами, грозовые руины с рунными кольцами. Поперечный уклон до 30° появляется только на поворотах с полным углом больше 30° и плавно уходит в ноль возле разрывов. Тесные повороты сглажены, чтобы края полотна не пересекались. Подъезды к разрывам имеют плавные профили длиной 36/42 м. Плиты ускорения и стартовая разметка повторяют поверхность дороги. Дальний фон удалён; широкие каменистые обочины и холмы поддерживают декорации. На всех изгибах стоят ограждения с обеих сторон.

Наклон машинки учитывает её направление относительно поверхности; в воздухе следует скорости полёта с ограничением угловой скорости. Коллизии используют выпуклый контур кузова и колёс без увеличенной прямоугольной оболочки; раздвижение работает постоянно, в том числе во время анимации столкновения. В руке показываются только бросаемые предметы, для которых работает прицел с шансом наведения 70%. При турбо огонь появляется в выходах выхлопных труб. Перед стартом можно выбрать от 1 до 10 кругов и от 0 до 8 соперников (до 9 машин вместе с игроком). При 8 соперниках один герой повторяется. Справа между мини-картой и скоростью показаны предмет и его название без панели. Скорость — число с км/ч, прицел — только скобки на цели. Выбор героя, трассы, кузница кузовов, улучшения и параметры заезда открываются отдельными окнами; управление подписано внизу главного меню. Всплывающие подсказки во время заезда убраны. После попадания газ работает сразу, эффективность разгона восстанавливается плавно. При отпускании дрифта сцепление плавно возвращается за доли секунды. В обычном повороте запас боковой силы шин превышает запрашиваемую рулём силу, поэтому направление движения следует рулю без ощущения льда. Очки и опыт сохраняются после финиша; покупки и улучшения — сразу. Рекорды трёхкруговых заездов сохраняют прежний формат; заезды другой длины их не перезаписывают. Прогресс Unreal хранится отдельно от browser localStorage, в `Saved/profile.json` проекта (у упакованного приложения — в пользовательской папке Saved, указанной в журнале игры).
...
- `tools/run.sh -WildDriftGallery -unattended -nosound` — 48 снимков всех 24 вариантов автомобилей спереди и сзади.
...
- `tools/run.sh -WildDriftVerify -unattended -nosound` — проверка ввода через PlayerController, прицела, броска, паузы, возврата, невидимости пассивных предметов в руке, выхлопа, прыжка/приземления и чтения/записи прогресса. Использует отдельный `verification-profile.json`; Также проверяет отсутствие прицела без цели, одиночный заезд, девять машин и настройки 1/10 кругов; пять окон меню и закрытие через Esc; успешный итог — `WILDDRIFT_INPUT_OK`.
...
Материалы металла, окрашенных панелей, кожи, кожи персонажей, резины и камня содержат отдельные карты цвета, нормалей и шероховатости, созданные детерминированно в `tools/native-materials.cjs`. Кузова собираются вокруг открытого кокпита; арочные крылья отделены от шин и не вращаются вместе с ними.
