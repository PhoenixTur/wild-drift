...
Нативная версия 0.2 для Apple silicon. В этой папке отдельный C++ проект UE 5.8; браузерная игра в корне репозитория остаётся самостоятельной.
...
Пять трасс различаются декорациями и погодой: пепельный ветер, дождливый лес, ледяной серпантин с метелью, кузня с шестернями и искрами, грозовые руины с рунными кольцами. Поперечный уклон рассчитывается на каждом повороте и плавно уходит в ноль возле разрывов. Дальний фон удалён; боковые площадки связывают декорации с дорогой.

Наклон машинки учитывает её направление относительно поверхности; в воздухе следует скорости полёта с ограничением угловой скорости. Коллизии используют габариты выбранного кузова и всех колёс; раздвижение работает постоянно, в том числе во время анимации столкновения. В руке показываются только бросаемые предметы, для которых работает прицел с шансом наведения 70%. При турбо огонь появляется в выходах выхлопных труб. Заезд: шесть гонщиков, три круга. Очки и опыт сохраняются после финиша; покупки и улучшения — сразу. Прогресс Unreal хранится отдельно от browser localStorage, в `Saved/profile.json` проекта (у упакованного приложения — в пользовательской папке Saved, указанной в журнале игры).
...
- `tools/run.sh -WildDriftVerify -unattended -nosound` — проверка ввода через PlayerController, прицела, броска, паузы, возврата, невидимости пассивных предметов в руке, выхлопа, прыжка/приземления и чтения/записи прогресса. Использует отдельный `verification-profile.json`; успешный итог — `WILDDRIFT_INPUT_OK`.
