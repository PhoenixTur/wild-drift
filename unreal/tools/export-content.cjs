...
const end=source.lastIndexOf('})();');source=source.slice(0,end)+'globalThis.port={kart,disposeGroup,buildWorld,placeKarts,updateAtmosphere,mesh,ellipsoid,softBox,tube,strut,mat,bakeStatic,label,get state(){return state;},get world(){return world;},get ambience(){return ambience;}};\n'+source.slice(end);vm.runInContext(source,context);
const art=require('./native-art.cjs')(THREE,context.port,context.RaceCore);context.RaceTracks.forEach(art.bankTrack);
...
const courses=[];for(let i=0;i<5;i++){context.port.buildWorld(i);const t=context.RaceTracks[i],file=`world-${t.id}.json`;art.reviseWorld(context.port.world,t,i);fs.writeFileSync(path.join(out,file),JSON.stringify(node(context.port.world)));courses.push({id:t.id,name:t.name,subtitle:t.meta.subtitle,length:t.length,width:t.width,fenceLane:t.fenceLane,points:t.segments.map(s=>s.a),gaps:t.gaps,fences:t.fences,banks:t.banks,camber:t.camber,pads:t.pads,pickups:t.pickups,overpasses:t.overpasses,bounds:t.bounds,theme:t.meta,file});console.log('Exported',file);}
fs.copyFileSync(path.join(root,'assets/fantasy-art.png'),path.join(out,'cover.png'));
const data={schema:1,source:crypto.createHash('sha256').update(read('game.js')+read('track.js')+read('race-core.js')+fs.readFileSync(__filename,'utf8')+read('unreal/tools/native-art.cjs')).digest('hex'),drivers:art.drivers,skins:context.RaceCore.SKINS,courses,vehicles,geometry,textures,materials};fs.writeFileSync(path.join(out,'catalog.json'),JSON.stringify(data));
